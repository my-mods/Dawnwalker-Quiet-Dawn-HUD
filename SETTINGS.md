...
Older On/Off panel settings are imported into the new opacity controls: Off becomes 0% and On becomes 100%. The upgrade adds the new keys while preserving existing settings, unknown keys and comments; it retains the original file as `settings.ini.before-sprint-prompt`. Any earlier `settings.ini.before-panel-opacity` or `settings.ini.before-time-panel` backup is retained. Compass opacity remains unchanged. Keep recovery files if an upgrade error is reported.
